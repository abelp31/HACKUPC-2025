...
io.on('connection', (socket: CustomSocket) => {
    ...
    socket.on('joinGame', ({ gameId, playerName }: { gameId: string, playerName: string }) => {
        console.log(`[joinGame] Attempt by ${socket.id}: GameID=${gameId}, PlayerName=${playerName}`);
        // Validation...
        ...
        // Successful Join...
        console.log(`[joinGame] Success: ${socket.id} | Player: ${playerName} | Game: ${gameId}`);
        socket.gameId = gameId;
        socket.playerName = playerName;
        game.players[socket.id] = { name: playerName, answers: [] };
        socket.join(gameId);
